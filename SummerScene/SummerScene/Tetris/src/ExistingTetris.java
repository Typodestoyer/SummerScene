...
public class Tetris extends JPanel implements KeyListener, ActionListener{
	...
	private static final int XSCORE = 100; //x-coordinate to display the score
	private static final int YSCORE = 570; //y-coordinate to display the score
	private static final int SOFTDROPDELAY = 50; //how many ms it takes for a piece to drop while holding s
	//Arrays
	private static int[][] pieceArray; //dimensions 4x2, 2 tells whether x or y coordinate, 4 tells which square of the piece you want
	private static int[][] piecePreviewArray; //same as above but for the sake of piece preview option
	private static int[][] board; //array that holds the tile statuses for the actual game, the imaginary space above the game, and the space in the sidebar
	...
	//type of piece, represented in character form
		...
		//s = backwards z-shape
		//S = square-shape
	private static ArrayList<Character> randomGenPiece = new ArrayList<Character>();
	private static char nextPiece;
	//type of piece for the preview and that will be chosen next
	private static int totalRowsCleared;
	...
	private static int rotationState;
	...
	private static javax.swing.Timer timerDrop;
	...
	public static void main(String args[])
	{
    	...
    	BORDER_COLOR = new Color(0,102,0);
		...
		speed = BASETIMEDELAY;
		//Declaring arrays
		pieceArray = new int[4][2];
		piecePreviewArray = new int[4][2];
		board = new int[WIDTH + 9][HEIGHT + 2];
		boardColors = new Color[WIDTH + 9][HEIGHT + 2];
		//Other declarations
		panel.addKeyListener(panel);
		panel.setFocusable(true);
		//Main
		char replay = 'n'; //decides whether game replays- no implementation yet
		...
		do
		{
    		...
    		while(gameOver == false)
			{
    			...
    			panel.repaint();
	 			...
	 			{
		  			timerDrop = new javax.swing.Timer(speed, panel); //panel now has a timer that activates once every speed ms.
	...
	public void paintComponent(Graphics g)
	{
		...
		g.drawString(Integer.toString(score),XSCORE,YSCORE);
	}
	...
	public void actionPerformed(ActionEvent evt) //every 1 second while the piece is dropping
    {
    	...
    	System.out.println(System.currentTimeMillis() + " " + (System.currentTimeMillis()-timeDiff) + " " + speed);
	...
	public static void previewPiece()
	{
	...
	public static void solidifyPiece()
	{
		for(int i = 0; i <= 3; i++)
		{
	...
	public static boolean checkDescent()
	{
		for(int i = 0; i <= 3; i ++)
		{
	...
	public static boolean checkLateralMovement(int direction)
	{
		for(int i = 0; i <= 3; i ++)
		{
	...
	public static void move(int direction) //-1 for left, 1 for right
	{
		for(int i = 0; i <= 3; i ++)
		{
			convertSquare(pieceArray[i][0],pieceArray[i][1],0,Color.BLACK);
	...
	public static Color typeToColor(char type)
	{
	...
	public static void convertSquare(int arrayX, int arrayY, int newType, Color newColor)
	{
		board[arrayX][arrayY] = newType;
		boardColors[arrayX][arrayY] = newColor;
	}
	public static void drawSquare(Graphics g, int arrayX, int arrayY, Color squareColor)
	{
